...
class AttackModule:
    ...
    def log_success(self, url, data_key, payload, status_code, vuln):
        ...
        self.mycursor.execute(f"TRUNCATE TABLE success_{vuln}")  # 테이블 초기화
        self.attackDB.commit()  # 변경 사항 적용
        ...
        self.mycursor.execute(f"INSERT INTO success_{vuln} (success) VALUES (%s)",
                            (f"URL: {url}, Data: {data_key}, Payload: {payload}, Status Code: {status_code}, Vuln: {vuln}",))
